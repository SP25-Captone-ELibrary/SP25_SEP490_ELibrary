    ...
    public class UserService : GenericService<User, UserDto, Guid>, IUserService<UserDto>
    {
        ...
        public async Task<IServiceResult> GetAllLibraryCardHolderAsync(ISpecification<User> spec)
        {
            try
            {
                ...
                var totalLibItemWithSpec =
                    await _unitOfWork.Repository<User, Guid>().CountAsync(cardHolderSpecification);
                // Count total page
                ...
                if (entities.Any())
                {
                    ...
                    // Pagination result 
                    var paginationResultDto = new PaginatedResultDto<LibraryCardHolderDto>(
                        userDtos.Select(u => u.ToLibraryCardHolderDto()),
                        cardHolderSpecification.PageIndex, cardHolderSpecification.PageSize, totalPage,
                        totalLibItemWithSpec);
            ...
            string email, UserDto userWithCard, string transactionToken)
        {
            try
            {
                ...
                if (!Equals(email, tokenExtractedData.Email))
                ...
                // Add necessary props
                userWithCard.LibraryCard.Barcode =
                    LibraryCardUtils.GenerateBarcode(_appSettings.LibraryCardBarcodePrefix);
            ...
            int? libraryCardPackageId)  // Use when register with cash payment
        {
            try
            {
                ...
                var isEmailExist =
                    (await _employeeService.AnyAsync(e => Equals(e.Email, processedByEmail))).Data is true;
                    ...
                    libraryCardPackageId == null) // Not include information of cash payment
                {
                    ...
                    var transSpec = new BaseSpecification<Transaction>(t =>
                        ...
                        t.TransactionStatus == TransactionStatus.Paid && // must be paid
                        t.TransactionType ==
                        TransactionType.LibraryCardRegister && // transaction type is lib card register
                        Equals(t.TransactionCode, transCode)); // transaction code
                    // Apply include
                ...
                if (isSaved)
                {
                    ...
                    if (isSent)
                    {
                        var successMsg = isEng
                            ? "Announcement email has sent to reader"
                            : "Email thông báo đã gửi đến độc giả";
                        // Msg: Register library card success
                        return new ServiceResult(ResultCodeConst.LibraryCard_Success0002,
                            await _msgService.GetMessageAsync(ResultCodeConst.LibraryCard_Success0002) +
                            $". {successMsg}");
                        ...
                        .AnyAsync(new BaseSpecification<User>(u =>
                            ...
                            (
                                ...
                                ((u.LibraryCardId != Guid.Empty && u.LibraryCard != null &&
                                  ...
                                  LibraryCardStatus.Pending) || // Already registered library card
                                 u.Invoices.Any() || // exist any invoices
                                 u.Transactions.Any() || // exist any transactions
                                 u.DigitalBorrows.Any() || // exist any transactions
                                 u.RefreshTokens.Any() || // has already signed in
        ...
        public async Task<IServiceResult> DeleteRangeLibraryCardHolderAsync(Guid[] userIds)
        {
            try
            {
                ...
                var deleteRangeCardRes =
                    await _libraryCardService.Value.DeleteRangeCardWithoutSaveChangesAsync(rangeCardIds);
            ...
            DuplicateHandle? duplicateHandle)
        {
            try
            {
                ...
                if (roleDto == null)
                {
                    ...
                    return new ServiceResult(ResultCodeConst.SYS_Fail0002,
                        StringUtils.Format(errMsg,
                            isEng ? "default role to process import" : "role mặc định để thực hiện import"));
                }
                ...
                if (detectDuplicateResult.Errors.Count != 0 &&
                    duplicateHandle == null) // Has not selected any handle options yet
                {
                ...
                foreach (var avatarImage in avatarImageFiles)
                {
                    ...
                    var validateResult = await
                        new ImageTypeValidator(lang.ToString() ?? SystemLanguage.English.ToString()).ValidateAsync(
                            avatarImage);
                    ...
                    var uploadResult =
                        (await _cloudService.UploadAsync(avatarImage, FileType.Image, ResourceType.BookImage))
                ...
                if (successRecords.Any())
                {
                    ...
                    var cardHolderList = successRecords.Select(r =>
                    {
                        ...
                        return r.ToUserDto(roleId: roleDto.RoleId, borrowSettings: _borrowSettings,
                            libraryCardAvatar: imageUrl);
                    }).ToList();

                    if (cardHolderList.Any())
                    {
                        // Add new cardholder
                        await _unitOfWork.Repository<User, Guid>()
                            .AddRangeAsync(_mapper.Map<List<User>>(cardHolderList));
                ...
                if (totalImported > 0 && totalFailed == 0)
                {
                    ...
                    message = StringUtils.Format(await _msgService.GetMessageAsync(ResultCodeConst.SYS_Success0005),
                ...
                if (totalImported > 0 && totalFailed > 0)
                {
                    ...
                    // Additional message (if any)
                    message = !string.IsNullOrEmpty(additionalMsg)
                        ? $"{message}, {additionalMsg} {failMessage}"
                        : message + failMessage;
                    return new ServiceResult(ResultCodeConst.SYS_Success0005, message,
                        Convert.ToBase64String(fileBytes));
                }
            ...
            for (int i = 0; i < records.Count; i++)
            {
                ...
                if (phoneNumber != null)
                {
                    ...
                    else
                    {
                        ...
                        var isPhoneExist =
                            await _unitOfWork.Repository<User, Guid>().AnyAsync(u => u.Phone == phoneNumber);
                        if (isPhoneExist)
                        {
                ...
                if (records[i].IsCreateLibraryCard)
                {
                    ...
                    {
                        errors.Add(isEng
                            ? "Barcode is required when create library card"
                            : "Yêu cầu nhập mã thẻ khi đánh dấu tạo thẻ đi kèm");
                    }
                    ...
                    else if (!Enum.TryParse(issuanceMethod, true, out LibraryCardIssuanceMethod issuanceMethodEnum))
                    {
                        ...
                        else if (issuanceMethod.ToLower() ==
                                 LibraryCardIssuanceMethod.InPerson.GetDescription().ToLower())
                        {
        ...
        )
        {
            ...
            for (int i = 0; i < records.Count; i++)
            {
                ...
                foreach (var field in scanningFields.Select(f => f.ToUpperInvariant()))
                {
                    ...
                    {
                        var title when title == nameof(LibraryCard.FullName).ToUpperInvariant() => record
                            .LibraryCardFullName?.Trim()
                            .ToUpperInvariant(),
...
//         private string GetLibraryCardExtensionEmailBody(LibraryCardDto cardDto, TransactionDto transactionDto,
