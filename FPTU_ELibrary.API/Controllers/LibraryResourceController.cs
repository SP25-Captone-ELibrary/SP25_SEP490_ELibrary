...
public class LibraryResourceController : ControllerBase
{
    ...
    //     return Ok(await _libraryResourceService.AddBookResourceToBookAsync(bookId, req.ToBookResourceDto(), email ?? string.Empty));
    // }
    //
    // [Authorize]
    ...
    //     // Retrieve user email from token
    //     var email = User.FindFirst(ClaimTypes.Email)?.Value;
    //     return Ok(await _libraryResourceService.UpdateAsync(id, req.ToBookResourceDto(), email ?? string.Empty));
    // }
    //
    // [Authorize]
